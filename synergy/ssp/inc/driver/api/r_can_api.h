...
typedef enum e_can_event
{
    ...
    CAN_EVENT_MAILBOX_OVERWRITE_OVERRUN,        ///< Mailbox has been overwritten or overrun.
} can_event_t;
...
typedef union
{
    ...
    struct st_status_b
    {
        ...
        uint32_t  reserved_4        : 13;   ///< Reserved bits.
    }  status_b;
} can_status_t;
...
{
    uint32_t                baud_rate_prescaler;             ///< Bit rate of CAN channel.
...
typedef struct st_can_api
{
     ...
     * - R_CAN_Control()
     * @param[in]   p_ctrl     Pointer to the CAN control block.
     * @param[in]   mode       Operation mode to switch to.
     */
