...
TX_THREAD uart_receiver;
void uart_receiver_create(void);
static void uart_receiver_func(ULONG thread_input);
/** Alignment requires using pragma for IAR. GCC is done through attribute. */
#if defined(__ICCARM__)
#pragma data_alignment = BSP_STACK_ALIGNMENT
#endif
static uint8_t uart_receiver_stack[1024] BSP_PLACE_IN_SECTION(".stack.uart_receiver") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
TX_EVENT_FLAGS_GROUP g_uart_status;
...
void uart_receiver_create(void)
{
    ...
    tx_event_flags_create (&g_uart_status, (CHAR *) "UART Status");

    tx_thread_create (&uart_receiver, (CHAR *) "UART Receiver", uart_receiver_func, (ULONG) NULL, &uart_receiver_stack,
